 ...
 * 
 * @return true wenn die Kamera diesen Frame rotiert wurde, sonst false.
 */
bool camera_mouseMoveCallback(Camera* camera, ProgContext ctx, float x, float y);
...
/**
 * Gibt den up-Vektor der Kamera zurück.
 * 
 * @param[in] camera Kamera, deren Blickrichtung ausgelesen wird
 * @param[out] up Vektor, in den das Ergebnis geschrieben werden soll.
 */
void camera_getUp(Camera* camera, vec3 up);

/**
 * Gibt den right-Vektor der Kamera zurück.
 * 
 * @param[in] camera Kamera, deren Blickrichtung ausgelesen wird
 * @param[out] right Vektor, in den das Ergebnis geschrieben werden soll.
 */
void camera_getRight(Camera* camera, vec3 right);
