 ...
 * @param[in] fontPath Pfad zum Font
 */
void gui_init(ProgContext ctx, const char* const fontPath);

/**
 * Rendert das GUI.  
 * Muss als letztes vor dem Double-Buffer Swap aufgerufen werden.
 * 
 * @note Die Position des Mauszeigers bestimmt, wohin Eingaben übertragen werden. Sobald die Maus über
 * einem GUI-Element ist, werden die Eingaben ausschließlich an das GUI und nicht an eure Inputverarbeitung
 * übergeben. Ist die Maus nicht über dem GUI, verhält es sich genau umgekehrt.
 * 
...
/**
 * Liefert die Textur-ID des RenderWindow
 * 
 * @param[in] renderWindow das RenderWindow
 * 
 * @return Textur-ID
 */
GLuint gui_getRenderWindowTexture(GuiRenderWindow renderWindow);
 ...
 * 
 * @param[in] ctx Programmkontext.
 * @param[in] array Die Auswahlmöglichkeiten
 ...
 * @param[in] stride,offset Wenn die Daten für die Anzeige Teil eines Struct Arrays sind, können diese wie bei glVertexAttribPointer() "herausgepickt" werden.
 *                          Bei einem einfachen String Array sind sie 0.
 * 
 ...
 * 
 * @warning @p stride und @p offset dürfen nur 0 sein, wenn @p array ein einfaches String Array ist. Ansonsten müssen die Werte korrekt gesetzt werden.
 */
int gui_dropdown(ProgContext ctx, void* array, int count, int selected, int item_height, struct nk_vec2 size, size_t stride, size_t offset);
...
/**
 * Zeigt ein Tooltip zu einem Element an. Dazu muss die Maus über dem Layout-Bereich des Elements sein.
 * 
 * @param[in] ctx Programmkontext
 * @param[in] text Inhalt des Tooltips
 * 
 * @note Diese Funktion muss *vor* dem Element ausgeführt werden.
 */
void gui_tooltip(ProgContext ctx, const char* const text);

/**
 * Liefert die vom GUI genutzte Skalierung
 * 
 * @param[in] ctx Programmkontext
 * 
 * @return Skalierungsfaktor
 */
float gui_getScale(ProgContext ctx);
