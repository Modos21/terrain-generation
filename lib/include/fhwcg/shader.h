 ...
 * gehängt wurden.
 * 
 * @note Bei Misserfolg gibt die Funktion eine Fehlermeldung aus und löscht den Shader in @p shader
 * 
 ...
 * 
 * @param[in,out] shader der Shader, der gelöscht werden soll. Die referenzierte Variable wird auf NULL gesetzt.
 */
...
/**
 * Liefert die Größe der local workgroups eines Compute Shaders
 * 
 * @param[in] shader der Compute Shader
 * @param[out] res die Größe der local workgroups, bleibt im Fehlerfall unverändert
 */
void shader_getWorkgroupSize(Shader* shader, ivec3 res);
...
*/\
void shader_set##typeName##N(Shader* shader, char* name, const type* values, GLsizei count); \
/**
 Übergibt einen typeName an einen Shader über eine Uniform Variable.  
...
/**
 ...
 @param[in] value der zu setzende Wert
 */\
void shader_set##typeName(Shader* shader, char* name, const type* value); \
/**
 Übergibt mehrere typeName an einen Shader über eine Uniform-Variable.
 ...
 *`"list[%d].%s", i, "foo"`* übergeben werden.
 ...
 @param[in] ... die weiteren Argumente für den Format-String in @p fmt
 */\
void shader_set##typeName##Fmt(Shader* shader, const type* value, char* fmt, ...);
